...
export interface AniversarianteSemana {
  ...
  is_today: boolean;
  whatsapp: string | null; // E.164 phone, e.g. +5581999999999
}
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AniversarianteSemana[]> => {
      ...
      const { data: members } = await supabase
        .from('members')
        .select('id, celula_id, whatsapp, profile:profiles!members_profile_id_fkey(name, avatar_url, birth_date)')
        .eq('is_active', true)
      ...
      for (const m of members) {
        ...
        if (weekDays.includes(bdMD)) {
          result.push({
            ...
            whatsapp: (m as any).whatsapp || null,
          });
        }
      }
