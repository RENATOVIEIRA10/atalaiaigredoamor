      ...
      <div className="max-w-2xl mx-auto px-4 py-6 sm:py-10">
        ...
        <Tabs defaultValue="fila" className="w-full">
          <TabsList className="bg-white/5 border border-white/10 mb-6 w-full">
            ...
            <TabsTrigger value="pipeline" className="flex-1 data-[state=active]:bg-[#C5A059]/20 data-[state=active]:text-[#C5A059]">
              <BarChart3 className="h-4 w-4 mr-1.5" />Pipeline
            </TabsTrigger>
            <TabsTrigger value="gargalos" className="flex-1 data-[state=active]:bg-[#C5A059]/20 data-[state=active]:text-[#C5A059]">
              <AlertTriangle className="h-4 w-4 mr-1.5" />Gargalos
            </TabsTrigger>
          </TabsList>

          <TabsContent value="fila"><FilaNovasVidas /></TabsContent>
...
function KPICards() {
  ...
  const cards = [
    ...
    { label: 'Encam. Hoje', value: stats.encaminhadasHoje, icon: Send, color: '#10B981' },
    { label: 'Reatribuir', value: stats.reatribuir, icon: RotateCcw, color: '#F97316' },
  ];
...
function PipelineView() {
  ...
  if (isLoading) return <div className="flex justify-center py-12"><Loader2 className="h-6 w-6 animate-spin" style={{ color: '#C5A059' }} /></div>;
      ...
      {pipeline.map(p => (
        <div key={p.status}>
          <button
            ...
            className="w-full flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/10 hover:border-[#C5A059]/30 transition-colors"
          >
            <div className="flex items-center gap-2">
              <Badge variant="outline" className={`text-[10px] ${p.color}`}>{p.label}</Badge>
            </div>
            <span className="text-lg font-bold" style={{ color: '#F4EDE4' }}>{p.count}</span>
          </button>
          {expandedStatus === p.status && (
            <div className="ml-4 mt-1 space-y-1">
              {p.vidas.map(nv => (
                <div key={nv.id} className="flex items-center justify-between p-2 rounded-lg bg-white/3 text-xs" style={{ color: '#B8B6B3' }}>
                  <span style={{ color: '#F4EDE4' }}>{nv.nome}</span>
                  <span>{differenceInDays(new Date(), new Date(nv.updated_at))}d</span>
                </div>
              ))}
            </div>
          )}
...
function GargalosView() {
    ...
    return (
      <Card className="bg-white/5 border-white/10">
        <CardContent className="py-12 text-center">
          <Heart className="h-10 w-10 mx-auto mb-3 opacity-30" style={{ color: '#10B981' }} />
          <p style={{ color: '#B8B6B3' }}>Nenhum gargalo identificado. Tudo fluindo! 🎉</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {gargalos.map(g => (
        <Card key={g.label} className="bg-white/5 border-white/10">
          <CardContent className="p-4 space-y-3">
            <div className="flex items-center gap-2">
              <g.icon className="h-4 w-4" style={{ color: g.color }} />
              <div>
                <p className="text-sm font-semibold" style={{ color: '#F4EDE4' }}>{g.label} ({g.vidas.length})</p>
                <p className="text-[11px]" style={{ color: '#B8B6B3' }}>{g.description}</p>
              </div>
            </div>
            <div className="space-y-1">
              {g.vidas.map(nv => (
                <div key={nv.id} className="flex items-center justify-between p-2 rounded-lg bg-white/3">
                  <div className="min-w-0">
                    <p className="text-xs font-medium truncate" style={{ color: '#F4EDE4' }}>{nv.nome}</p>
                    <p className="text-[10px]" style={{ color: '#B8B6B3' }}>
                  ...
                  {nv.status === 'reatribuir' && (
                    <Button
                      ...
                      style={{ background: '#C5A059', color: '#1A2F4B' }}
  ...
  return (
    <div className="space-y-4">
      <div className="relative">
        ...
        <Input placeholder="Buscar por nome..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 bg-white/5 border-white/10 text-[#F4EDE4]" />
      </div>
      ...
      {triagemId && <TriagemDialog vidaId={triagemId} novasVidas={novasVidas || []} onClose={() => setTriagemId(null)} />}
    </div>
  );
}
