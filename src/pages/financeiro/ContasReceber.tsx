...
import { Plus, Search, CheckCircle, Trash2, Edit, ArrowUpRight } from 'lucide-react';
...
export default function ContasReceber() {
  ...
  const { data: contas, isLoading } = useFinContasReceber({ status: statusFilter });
  const { markReceived, remove } = useFinContaReceberMutations();
  ...
  const filtered = (contas || [])
    ...
    .map((c) => ({
      ...c,
      displayStatus: c.status === 'pendente' && c.data_prevista < today ? 'atrasado' : c.status,
    }));
                        ...
                        <TableRow key={c.id} className="group">
                          ...
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              {c.status !== 'recebido' && (
                                <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => markReceived.mutate(c.id)}>
